...
version = args['dataset'].split("multiwoz")[-1] # default:'2.1'
...
def loadData():
    ...
    if version == '2.0':
        ...
        if not os.path.exists("data2.0"):
            os.makedirs("data2.0")
            os.makedirs("data2.0/multi-woz")
    
        if not os.path.exists(data_url):
            print("Downloading and unzipping the MultiWOZ dataset")
            resp = urllib.request.urlopen(dataset_url)
            zip_ref = ZipFile(BytesIO(resp.read()))
            zip_ref.extractall("data2.0/multi-woz")
            zip_ref.close()
            shutil.copy('data2.0/multi-woz/MULTIWOZ2 2/data.json', 'data2.0/multi-woz/')
            shutil.copy('data2.0/multi-woz/MULTIWOZ2 2/valListFile.json', 'data2.0/multi-woz/')
            shutil.copy('data2.0/multi-woz/MULTIWOZ2 2/testListFile.json', 'data2.0/multi-woz/')
            shutil.copy('data2.0/multi-woz/MULTIWOZ2 2/dialogue_acts.json', 'data2.0/multi-woz/')    

    else: # "2.1"
        dataset_url = 'https://www.repository.cam.ac.uk/bitstream/handle/1810/294507/MULTIWOZ2.1.zip?sequence=1&isAllowed=y'
        if not os.path.exists("data2.1"):
            os.makedirs("data2.1")
            os.makedirs("data2.1/multi-woz")
    
        if not os.path.exists(data_url):
            print("Downloading and unzipping the MultiWOZ dataset")
            resp = urllib.request.urlopen(dataset_url)
            zip_ref = ZipFile(BytesIO(resp.read()))
            zip_ref.extractall("data2.1/multi-woz")
            zip_ref.close()
            shutil.copy('data2.1/multi-woz/MULTIWOZ2.1/data.json', 'data2.1/multi-woz/')
            shutil.copy('data2.1/multi-woz/MULTIWOZ2.1/valListFile.json', 'data2.1/multi-woz/')
            shutil.copy('data2.1/multi-woz/MULTIWOZ2.1/testListFile.json', 'data2.1/multi-woz/')
            shutil.copy('data2.1/multi-woz/MULTIWOZ2.1/dialogue_acts.json', 'data2.1/multi-woz/')  
